...
`make qc` launches a per-patient interactive Plotly Dash trajectory viewer at http://localhost:8050. Pick a site + a `hospitalization_id` and inspect a 5-panel timeline (sedatives, pressors, assessments, resp, vitals) with clinical event overlays. Loads only the selected patient's data — full-cohort memory is never materialized.
