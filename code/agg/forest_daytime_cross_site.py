...
absolute-rate predictors instead of the night–day diff predictors. Reads
each site's `output_to_share/{site}/models/models_coeffs.csv`. Same
layout, same spec encoding, same outcomes — destined for the manuscript's
supplement so reviewers can see whether the dose-level signal diverges
from the night–day variation signal.
...
exposed" framing), so the daytime-row scaling is conceptually consistent
with the night-day diff scaling.
...
logger = get_logger("epi_sedation.agg.forest_daytime")
...
# model_type matches the meta-analysis pool scope (logit_asym for extub,
# gee for SBT) so per-site dots and the Pooled diamond reflect the same fit.
...
# 3 sedatives, daytime continuous-rate version. SHORT labels (no unit
# suffix — those are in the headline italic line).
...
PRES_UNITS_LINE: dict[str, str] = {
    ...
    "p10_p90":  "OR per (10th→90th percentile shift, among-the-exposed); shift sizes vary by site",
}
...
def _render(
    ...
    pooled_y = 0
    site_y = {s: (n_sites - i) for i, s in enumerate(sites)}
    ...
    add_salient_headline(
        fig,
        title="Effect of daytime continuous dose rate on next-day liberation",
        subtitle=(
...
def main() -> None:
    ...
    for presentation, suffix in [("per_unit", ""), ("p10_p90", "_audit")]:
        ...
        save_agg_fig(fig, f"forest_daytime_cross_site{suffix}")
        plt.close(fig)
