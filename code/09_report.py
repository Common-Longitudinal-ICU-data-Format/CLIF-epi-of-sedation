...
def _():
    ...
    **Pages (in order):**
    ...
    10. Marginal effects — SBT Done Next Day (GEE)

    11. Marginal effects — Successful Extubation (GEE)

    12. Marginal effects — Successful Extubation (Logit)
    """)
    return
...
def _(
    ...
    with PdfPages(_pdf_path) as _pdf:
        ...
        # Pages 7-9: Model comparison tables (stargazer-style)
        add_stargazer_table(
            _pdf, sbt_gee_df,
        ...
        # Pages 10-12: Marginal effect plots (2×3 grid per outcome × model_type).
        # PNGs are generated by 08_models.py from the `sofa` spec; we just embed
        # them here as regular image pages. If a PNG is missing, drop a placeholder
        # text page so the PDF still compiles.
        for _label, _outcome_short, _mt in [
            ...
            if os.path.exists(_me_path):
                add_image_page(_pdf, _me_path, f"Marginal Effects — {_label}")
            else:
                add_text_page(
                    _pdf,
                    f"Marginal Effects — {_label}",
                    [
